...
def main():
    ...
    # 2. Read the template Phase 3 config
    config_path = Path("configs/phase3_wgisd_domain_guided.json")
    if not config_path.exists():
        print(f"Error: Template config not found: {config_path}")
        sys.exit(1)
    ...
    # 4. Save to a new config file
    output_config_path = Path("configs/phase3_wgisd_agri_adapted.json")
    with open(output_config_path, "w") as f:
        json.dump(config, f, indent=4)

    print(f"Created adapted config: {output_config_path}")
    print("Ready for Phase 3 execution.")
